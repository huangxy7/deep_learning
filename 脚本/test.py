...
model_weights = '/home/jsj/xytest/model/iter_13000.caffemodel' # 修改成你的caffemodel文件的路径
...
with open('/home/jsj/xytest/val_bak.txt') as image_list: # 修改成你要测试的txt文件的路径，这个txt文件的内容一般是：每行表示图像的路径，然后空格，然后是标签，也就是说每行都是两列
    with open('/home/jsj/xytest/test_res.txt','w') as result: # 如果你想把预测的结果写到一个txt文件中，那么把这个路径修改成你想保存这个txt文件的路径
        count_right=0
        count_all=0
        while 1:
